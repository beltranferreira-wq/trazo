...
wss.on('connection', ws => {
  ...
  ws.on('message', async raw => {
    ...
    if (msg.type === 'position') {
      await pool.query('UPDATE shipments SET courier_lat=$1,courier_lng=$2,courier_active=TRUE,position_updated_at=NOW() WHERE tracking=$3',[msg.lat,msg.lng,msg.code]);
...
app.post('/api/shipments', async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [id,tracking,b.product.trim(),emojis[Math.floor(Math.random()*emojis.length)],b.carrier||'Repartidor',b.recipient||'Cliente',b.phone||'',b.courierPhone||'',b.originName||'',parseFloat(b.originLat)||null,parseFloat(b.originLng)||null,b.destName||'',parseFloat(b.destLat)||null,parseFloat(b.destLng)||null,dates]
  );
...
app.get('/share/:code', async (req, res) => {
  ...
  const desc = s ? `${STEPS[s.currentStep]||'En proceso'} · Para: ${s.recipient}${s.destName?' · '+s.destName.split(',')[0]:''}` : 'Seguí tu pedido en tiempo real.';
  ...
  res.send(`<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>${title}</title><meta property="og:title" content="${title}"><meta property="og:description" content="${desc}"><meta property="og:image" content="${imageUrl}"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta property="og:url" content="${liveUrl}"><meta property="og:type" content="website"><meta name="twitter:card" content="summary_large_image"><meta http-equiv="refresh" content="0;url=${liveUrl}"><script>window.location.replace('${liveUrl}');</script></head><body style="background:#15182B;color:#fff;font-family:Arial;display:flex;align-items:center;justify-content:center;height:100vh;margin:0"><p>Redirigiendo...</p></body></html>`);
});

const { createCanvas, GlobalFonts } = require('@napi-rs/canvas');
const FONT_PATHS = [['/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf','TrazoBold'],['/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf','TrazoBold'],['/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf','TrazoReg'],['/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf','TrazoReg']];
FONT_PATHS.forEach(([p,name]) => { try { if(fs.existsSync(p)) { GlobalFonts.registerFromPath(p,name); console.log('Font:',name); } } catch(e){} });
...
app.get('/og-image/:code', async (req, res) => {
  const code = req.params.code;
  if (ogCache.has(code)) { const {buf,ts}=ogCache.get(code); if(Date.now()-ts<60000){res.setHeader('Content-Type','image/png');res.setHeader('Cache-Control','public,max-age=60');return res.send(buf);} }
  const { rows } = await pool.query('SELECT * FROM shipments WHERE tracking=$1',[code]);
  const s = rows[0] ? rowToShipment(rows[0]) : null;
  ...
  const recipient = s?s.recipient:''; const dest = s&&s.destName?s.destName.split(',')[0]:'';
  try {
    const W=1200,H=630,canvas=createCanvas(W,H),ctx=canvas.getContext('2d');
    const bold=n=>`bold ${n}px TrazoBold`, reg=n=>`${n}px TrazoReg`;
    ctx.fillStyle='#15182B';ctx.fillRect(0,0,W,H);ctx.fillStyle='#0D0F1D';ctx.fillRect(0,0,W,200);
    ctx.fillStyle='#FF5A36';ctx.fillRect(0,0,12,H);ctx.fillRect(60,52,140,30);
...
app.post('/api/courier-position', async (req, res) => {
  ...
  await pool.query('UPDATE shipments SET courier_lat=$1,courier_lng=$2,courier_active=TRUE,position_updated_at=NOW() WHERE tracking=$3',[lat,lng,code]);
...
server.listen(PORT, '0.0.0.0', async () => {
  ...
  try { await initDB(); console.log('DB lista.'); } catch(e) { console.error('DB error:', e.message); }
  if (process.env.RENDER) {
    const https = require('https');
    const selfUrl = process.env.RENDER_EXTERNAL_URL || 'https://trazo-hbrf.onrender.com';
    setInterval(() => { https.get(selfUrl+'/health',(r)=>console.log('Keep-alive:',r.statusCode)).on('error',()=>{}); }, 10*60*1000);
